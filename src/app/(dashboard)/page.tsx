...
export default function DashboardPage() {
  ...
  return <div className="space-y-6 pb-10"><header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[.18em] text-sky-600">Operations overview</p><h1 className="mt-1 text-2xl font-bold text-slate-950">{overview.greeting}</h1><p className="mt-1 text-sm text-slate-500">{overview.subtitle_date}</p></div><div className="relative"><button onClick={() => setActionsOpen((open) => !open)} className="flex h-10 items-center gap-2 rounded bg-sky-500 px-4 text-sm font-semibold text-white"><MdAdd />Create or add</button>{actionsOpen && <div className="absolute right-0 z-20 mt-2 w-56 rounded border bg-white p-1.5 shadow">{[["Create a shift", "/roster", MdCalendarToday], ["Add client or location", "/clients", MdBusiness], ["Bulk import data", "/users", MdUploadFile]].map(([label, href, Icon]) => <Link key={label as string} href={href as string} className="flex items-center gap-3 rounded px-3 py-2.5 text-sm hover:bg-slate-50"><Icon className="text-sky-500" />{label as string}</Link>)}</div>}</div></header>
    ...
    <section className={`rounded border p-4 ${overview.attention_banner.people_need_attention_count > 0 ? "border-red-200 bg-red-50" : "border-slate-200 bg-white"}`}>
      <div className="flex flex-wrap items-center gap-4">
        <span className={`flex h-11 w-11 items-center justify-center rounded text-white ${overview.attention_banner.people_need_attention_count > 0 ? "bg-red-500" : "bg-emerald-500"}`}>
        ...
        <div className="flex-1">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className={`rounded px-2 py-1 text-xs font-semibold ${overview.attention_banner.people_need_attention_count > 0 ? "bg-red-100 text-red-700" : "bg-emerald-100 text-emerald-700"}`}>
              {overview.attention_banner.people_need_attention_count > 0 ? overview.attention_banner.badge_text : "All on time"}
            </span>
          </div>
          <p className={`text-sm ${overview.attention_banner.people_need_attention_count > 0 ? "text-red-700" : "text-slate-500"}`}>
        ...
        <div className="flex flex-wrap gap-2">
          {attentionPills.length > 0 ? (
            attentionPills.map((item) => (
              <a key={item.worker_id} href={`tel:${item.phone_number}`} title={item.worker_name} className="flex items-center gap-2 rounded border border-red-200 bg-white px-3 py-2 text-xs font-semibold text-red-700">
                <img src="/avatar-placeholder.svg" alt={item.worker_name} className="h-6 w-6 rounded-full" />
                {item.late_duration_text}
                <MdCall />
              </a>
            ))
          ) : (
            <span className="text-xs text-emerald-600 font-medium bg-emerald-50 border border-emerald-200 rounded-lg px-3 py-2">
    ...
    <section className={`rounded border p-4 ${fallingBehind.length > 0 ? "border-amber-200 bg-amber-50" : "border-slate-200 bg-white"}`}>
      <h2 className={`font-bold ${fallingBehind.length > 0 ? "text-amber-950" : "text-slate-900"}`}>
        Falling behind schedule <span className={`ml-2 rounded px-2 py-1 text-xs ${fallingBehind.length > 0 ? "bg-amber-200 text-amber-900" : "bg-slate-100 text-slate-600"}`}>{fallingBehind.length} alerts</span>
      </h2>
      ...
      {fallingBehind.length > 0 && (
        <div className="mt-3 grid gap-2 lg:grid-cols-2">
          {fallingBehind.slice(0, 6).map((item) => {
            ...
            return (
              <div key={`${item.shift_id}-${item.worker_id}`} className="rounded border border-amber-200 bg-white p-3">
                <div className="flex items-center gap-3">
                  <img src={item.worker_profile_picture || "/avatar-placeholder.svg"} alt={item.worker_name} className="h-9 w-9 rounded-full" />
                  <div className="flex-1">
                    <b className="text-sm">{item.worker_name}</b>
                    <p className="text-xs text-slate-500">{item.location_name} · {item.shift_start_time}–{item.shift_end_time}</p>
                  </div>
                  <b className="text-xs text-amber-700">{item.progress_percentage || `${progress}%`}</b>
                </div>
                <div className="mt-2 h-1.5 rounded bg-slate-100">
                  <div className="h-full rounded bg-amber-500" style={{ width: `${Math.min(progress, 100)}%` }} />
                </div>
              </div>
            );
          })}
