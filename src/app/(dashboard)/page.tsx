...
const progressAlerts = [
  { worker: "Lisa Visser", location: "NH Hotel Amsterdam", completed: 3, total: 4, remaining: 22, phone: "+31612345678" },
  { worker: "Sophie de Boer", location: "UMC Utrecht · Floor 2", completed: 8, total: 12, remaining: 28, phone: "+31622446688" },
];
  ...
  return (
    <div className="space-y-6 pb-10">
      ...
      <section className="rounded border border-amber-200 bg-amber-50 p-4 shadow-sm">
        <div className="mb-3"><div className="flex items-center gap-2"><MdAccessTime className="text-xl text-amber-600" /><h2 className="font-bold text-amber-950">Falling behind schedule</h2><span className="rounded-full bg-amber-200 px-2 py-0.5 text-xs font-bold text-amber-800">{progressAlerts.length} alerts</span></div><p className="mt-1 text-xs text-amber-700">Shift is nearly over and assigned objects may not finish on time.</p></div>
        <div className="grid gap-2 lg:grid-cols-2">{progressAlerts.map(alert => <div key={alert.worker} className="flex flex-wrap items-center gap-3 rounded border border-amber-200 bg-white p-3"><img src="/avatar-placeholder.svg" alt={alert.worker} className="h-9 w-9 rounded-full border object-cover" /><div className="min-w-0 flex-1"><p className="text-sm font-bold text-slate-900">{alert.worker}</p><p className="truncate text-xs text-slate-500">{alert.location} · {alert.completed}/{alert.total} objects done</p><div className="mt-2 h-1.5 overflow-hidden rounded-full bg-slate-100"><div className="h-full bg-amber-500" style={{width:`${alert.completed / alert.total * 100}%`}} /></div></div><span className="text-xs font-bold text-amber-700">{alert.remaining}m left</span><a href={`tel:${alert.phone}`} className="flex h-8 items-center gap-1 rounded bg-emerald-600 px-3 text-xs font-bold text-white"><MdCall /> Call</a></div>)}</div>
      </section>
