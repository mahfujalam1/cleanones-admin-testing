...
export default function LoginPage() {
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    if (apiRole !== 'MANAGER' && apiRole !== 'SUPER_ADMIN') { setError(t.subtitle); return; }
    ...
    <div className="w-full max-w-md overflow-hidden rounded border border-slate-200 bg-white shadow-[0_2px_8px_rgba(15,23,42,0.06)]">
      <div className="p-5 sm:p-8">
        <div className="mb-8 flex flex-col items-center text-center">
          ...
          <h2 className="text-xl font-bold tracking-tight text-slate-900">{t.welcome}</h2>
        ...
        <form onSubmit={handleSignIn} className="space-y-4">
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-slate-800">{t.password}</label>
          ...
          <div className="flex items-center justify-between pt-0.5">
            <label className="flex cursor-pointer items-center gap-2 text-xs text-slate-700">
              <input type="checkbox" checked={rememberMe} onChange={(event) => setRememberMe(event.target.checked)} className="h-4 w-4 rounded border-slate-300 accent-primary" /> {t.rememberMe}
            </label>
