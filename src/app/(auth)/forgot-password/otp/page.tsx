...
export default function OTPPage() {
  ...
  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    const code = otp.join('');
    if (code.length !== 6) return setError(t.completeOtp);
  ...
  const handleResend = async () => {
    if (!email) {
      ...
      if (!stored) return setError(t.restartReset);
    }
    ...
    setResending(false);
    if (result.success) {
      setInfo(typeof result.data === 'string' ? result.data : t.otpSent);
    } else {
    ...
    <div className="w-full max-w-md space-y-5 rounded border border-slate-200 bg-white p-6 sm:p-8">
      <div className="flex select-none flex-col items-center">
        ...
        <p className="mt-1 text-center text-xs text-slate-500">
          {t.otpDescription} {email ? <span className="font-semibold text-slate-700">{email}</span> : t.emailFallback}. Type the 6-digit code below.
        </p>
      </div>
      <form onSubmit={submit} className="space-y-4">
        ...
        <button type="submit" className="h-10 w-full cursor-pointer rounded bg-primary text-sm font-semibold text-white shadow-sm hover:bg-[#0284c7]">{t.verifyCode}</button>
      </form>
      <p className="text-center text-[11px] text-slate-400">Didn&apos;t receive the code? <button type="button" onClick={handleResend} disabled={resending} className="font-semibold text-primary hover:underline disabled:opacity-60">{resending ? t.sending : t.resendCode}</button></p>
    </div>
  );
}
