...
export default function ResetPasswordPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    const email = sessionStorage.getItem('cleanones-reset-email'); const otpCode = sessionStorage.getItem('cleanones-reset-otp');
    if (!email || !otpCode) return setError('Reset session expired. Please request a new OTP.');
    setLoading(true); const result = await resetPassword({ email, otp_code: otpCode, new_password: password }); setLoading(false);
    if (!result.success) return setError(result.error);
    sessionStorage.removeItem('cleanones-reset-email'); sessionStorage.removeItem('cleanones-reset-otp'); router.push('/login');
  };

  const fieldClass = "h-10 w-full rounded border border-slate-200 bg-white px-3 text-sm text-slate-900 shadow-sm outline-none transition-colors placeholder:text-slate-400 focus:border-primary focus:ring-2 focus:ring-primary/10";
  return (
