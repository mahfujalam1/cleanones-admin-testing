    ...
    <div className="flex flex-col h-full space-y-4 animate-in fade-in duration-300">
      ...
      <div className="flex items-center justify-between bg-white p-3 rounded-lg border border-gray-200 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <h2 className="text-lg font-semibold text-gray-800 min-w-[250px]">{formatDateRange()}</h2>
        </div>
        ...
        <div className="flex items-center gap-4">
          <div className="flex bg-gray-100 p-1 rounded-md text-sm font-medium">
            {(['Day', 'Week', 'Month'] as const).map(v => (
              <button
                key={v}
                onClick={() => setView(v)}
                className={`px-4 py-1.5 rounded transition-all duration-200 cursor-pointer ${view === v ? 'bg-[#0ea5e9] text-white shadow-sm' : 'text-gray-600 hover:text-gray-900'}`}
              >
                {v}
              </button>
            ))}
          ...
          <button
            ...
            className="bg-[#0ea5e9] hover:bg-[#0284c7] text-white px-4 py-1.5 rounded text-sm font-medium shadow-sm transition-colors flex items-center gap-1 cursor-pointer"
          >
