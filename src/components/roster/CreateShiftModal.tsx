  ...
  useEffect(() => {
    void getRosterWorkers({ targetDate: date }).then((result) => {
      ...
      setWorkers(list.map((item: any) => ({
        ...
        // Backend (`getRosterWorkerDropdowns`) only ever returns 'on_shift' or 'available' here —
        // 'active' was never a value it sends, so on_shift workers were silently mislabeled
        // "Off Duty" instead of the amber "On Shift" state.
        status: (item.status || '').toLowerCase() === 'on_shift' ? 'On Shift' : (item.status || '').toLowerCase() === 'available' ? 'Available' : 'Off Duty',
        color: 'bg-sky-500'
      })));
    });
  }, [date]);
