...
export default function ClientOverview({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fields = [[ui.clientId, id], [ui.companyName, client?.company_name || client?.name], [ui.contractStatus, client?.contract_status], [ui.licenceExpiryDate, client?.licence_expiration_date?.slice(0, 10)], [ui.locations, data?.meta.total], [ui.email, client?.email], [ui.phone, client?.phone]];
