...
export function AddClientModal({ onClose, onAdd }: AddClientModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!licenseExpirationDate) return setError(t.common.required);
    ...
    >
      ...
      >
        ...
        <div className="flex items-start justify-between px-6 pt-6 pb-2">
          <div>
            <h2 className="text-lg font-bold text-gray-900">{t.clients.addClient}</h2>
            <p className="text-xs text-gray-400 mt-0.5">{t.clients.title}</p>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="px-6 py-5 space-y-4">
          {/* Company Name */}
          <div>
            <label className="text-xs font-semibold text-gray-700 mb-1.5 block">{t.settings.companyName} *</label>
            <input
              type="text"
              required
              placeholder={p.name}
          ...
          <div><label className="text-xs font-semibold text-gray-700 mb-1.5 block">{t.roster.date} *</label><input type="date" required value={licenseExpirationDate} onChange={(event) => setLicenseExpirationDate(event.target.value)} className="w-full h-10 rounded border border-gray-300 bg-white px-3 text-sm" /></div>
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-5 border-t border-gray-100">
          ...
          >
            {saving ? t.settings.saving : t.clients.addClient}
          </button>
        </div>
      </form>
    </div>
