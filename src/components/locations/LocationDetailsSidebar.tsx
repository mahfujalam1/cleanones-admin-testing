...
import { getLocationCleaningPlans, getLocationOverview, getLocationRooms, type LocationOverview, type RoomsTab } from "@/services/actions/locations";
...
export function LocationDetailSidebar({ location, onClose }: LocationDetailSidebarProps) {
  ...
  const [roomsData, setRoomsData] = useState<RoomsTab | null>(null);
  const [plans, setPlans] = useState<unknown>(null);
  const [error, setError] = useState("");
  useEffect(() => { setError(""); if (tab === "overview") void getLocationOverview(location.id).then((result) => result.success ? setOverview(result.data) : setError(result.error)); if (tab === "rooms") void getLocationRooms(location.id).then((result) => result.success ? setRoomsData(result.data) : setError(result.error)); if (tab === "plans") void getLocationCleaningPlans(location.id).then((result) => result.success ? setPlans(result.data) : setError(result.error)); }, [location.id, tab]);
  return <>
    ...
    <div className="fixed right-0 top-0 z-[65] flex h-dvh w-full max-w-[430px] flex-col border-l border-gray-200 bg-white animate-in slide-in-from-right duration-200">
      ...
      <div className="flex-1 overflow-y-auto p-4">
        ...
        {tab === "rooms" && <div className="space-y-2">
          ...
          {(roomsData?.rooms ?? []).map((room) => <div key={room.room_id} className="flex items-center gap-3 rounded border border-gray-200 p-3"><span className="flex h-8 w-8 items-center justify-center rounded bg-sky-50 text-sky-500"><MdMeetingRoom /></span><div className="flex-1"><p className="text-xs font-semibold text-slate-700">{room.title}</p><p className="text-[10px] text-slate-500">Floor {room.floor} · {room.cleaning_type_label}</p></div><span className="text-[10px] text-emerald-600">{room.status}</span></div>)}
        ...
        {tab === "plans" && <div className="space-y-2">
          ...
          {plans == null ? <DetailSkeleton blocks={3} /> : <div className="rounded border border-gray-200 p-3 text-xs text-slate-600">{typeof plans === "string" ? plans : JSON.stringify(plans)}</div>}
