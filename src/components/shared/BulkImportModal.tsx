...
export function BulkImportModal({ onClose, mode, onImported }: { onClose: () => void; mode?: "locations" | "workers"; onImported?: () => void }) {
  ...
  return (
    ...
    >
      <div onMouseDown={(e) => e.stopPropagation()} className={`w-full max-w-lg rounded-md bg-white p-6 shadow ${jumpClassName}`}>
      <div className="flex items-start justify-between"><div><h2 className="text-xl font-bold">{t.common.bulkImport}</h2><p className="mt-1 text-sm text-slate-500">{t.dashboard.bulkImportData}</p></div><button onClick={onClose} className="rounded p-2 hover:bg-slate-100"><MdClose /></button></div>
      {done ? <div className="my-6 text-center"><MdCheckCircle className="mx-auto text-4xl text-emerald-500" /><h3 className="mt-3 font-bold">{t.common.status}</h3><p className="mt-1 text-sm text-slate-500">{file?.name}</p></div> : <>
        <label className="my-5 flex min-h-44 flex-col items-center justify-center rounded border-2 border-dashed border-slate-200 bg-slate-50 p-6 text-center hover:border-sky-400">
          <MdUploadFile className="text-3xl text-sky-500" /><span className="mt-2 text-sm font-bold">{file ? file.name : t.common.bulkImport}</span><span className="mt-1 text-xs text-slate-500">{t.common.required}</span>
      ...
      <div className="mt-6 flex justify-end gap-2 border-t pt-4">
        <button onClick={onClose} className="rounded border px-4 py-2 text-sm font-semibold">{t.common.cancel}</button>
        {!done && <button disabled={!file || loading} onClick={runImport} className="rounded bg-sky-500 px-4 py-2 text-sm font-bold text-white disabled:opacity-40">{loading ? t.settings.saving : t.common.save}</button>}
        {done && <button onClick={onImported ?? onClose} className="rounded bg-sky-500 px-4 py-2 text-sm font-bold text-white">{t.common.next}</button>}
      </div>
    </div>
  </div>
  );
